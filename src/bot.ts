...
export function createBot(token: string) {
  ...
  bot.command("recent", async (ctx) => {
    ...
    try {
      ...
      const items = data.data.slice(0, 30); // show 30 most recent
      ...
      const itemsWithInfo = await Promise.all(
        items.map(async (item: any) => ({
          ...
          delegatorInfo: item.delegator
            ? await getENSInfo(item.delegator)
            : null,
        }))
      );
      ...
      for (const message of messages) {
        ...
        console.log("sending messages in for loop");
